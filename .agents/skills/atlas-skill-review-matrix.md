...
|---|---|---|---|---|---|
| `atlas.prd-extraction` | Extract atomic, source-grounded assertions from one immutable PRD or Addendum. | Artifact metadata and page text; targeted state is optional context. | Candidate assertions, source-statement inventory, and questions with skill/mode provenance. | Cannot create truth, projections, revisions, approvals, or commits. | Proposal-not-truth; exact artifact/page/quote evidence; full page coverage; ambiguity becomes a question. |
| `atlas.fixture-repository` | Assemble an append-only repository candidate. | Project ID, source artifacts, requested scenario, optional accepted base, and extraction results when extraction-backed. | Candidate branches, revisions, and branch/HEAD-keyed materialized states. Every revision records producing skill ID, skill version, and actual mode. | Cannot approve, commit, mutate storage, or move HEAD. | Stable IDs; branch isolation; extraction handoff required when applicable; every `headRevisionId` resolves; provenance is present. |
