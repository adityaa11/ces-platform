...
| Compact-mobile profile/session control | Opened navigation drawer, then opened the Account menu modal sheet; inspected its close control, role, theme controls, account actions, Escape, and outside/backdrop dismissal. | Dark, Light | Compact mobile (573 px) | The account sheet retains a distinct heading and action grouping at the bottom of the compact layout. | Rendered modal exposed `role="dialog"`, `aria-modal`, a labelled heading, and explicit Close dialog control; the Dialog test verifies focus entry/containment, Escape, outside/backdrop dismissal, and trigger-focus restoration. | Pass — the mobile navigation trigger revealed the avatar/profile trigger; Account menu rendered as a dimming modal bottom sheet over the underlying workspace, rather than a clipped popover; activating its backdrop closes the sheet without treating panel interaction as dismissal. |
...
- **New mandatory state for this component:** Inspect desktop popover and compact-mobile modal sheet separately in both themes, including open/closed, Escape, outside dismissal, focus restoration, clipping, and connected workspace context.
