...
Example Requirement Package input:
...
Every Policy Manifest uses one `obligations` array for mandatory, conditional,
prohibited, resolved, blocked, and conflicting policies. For example, a resolved
entry has the implemented shape:
...
```json
{
  ...
  "reasons": ["Business rule BR-USER-032"],
  "evidence": [],
  "source_rule_ids": [],
  "business_rule_ids": ["BR-USER-032"],
  "missing_inputs": []
}
```

Prohibitions use `requirement_level: "prohibited"`; unresolved and conflicting
entries remain in the same array with `resolution_state: "blocked"` or
`"conflict"`. Separate `prohibitions` and `blocked_obligations` arrays are not
part of the Phase 1 schema.
...
assurance:
  ...
  data_classes:
    - personal
    - sensitive
  delivery_semantics: asynchronous
...
ces compile-adapter \
  ...
  --output generated/adapters/laravel
```

Expected output:

```text
generated/adapters/laravel/
├── implementation-plan.json
├── implementation-task.md
├── test-manifest.json
└── verification-manifest.json
