...
- **State:** `planned` — SUS-001 through SUS-004 remain approved and frozen; SUS-005 is a new user-authorized UI refinement and remains planned until `go`.
...
| 4 | [SUS-004](SUS-004-worker-runtime-sign-up-compatibility.md) / `SUS-BATCH-04` | `approved` | SUS-001 through SUS-003 approved; runtime-compatibility scope change approved | Does the real Vinext worker runtime complete sign-up without dynamic-code/CSP workarounds while preserving the Better Auth and Atlas boundaries? |
| 5 | [SUS-005](SUS-005-sign-up-guidance-and-error-states.md) / `SUS-BATCH-05` | `planned` | SUS-004 `PASS` | Does the sign-up form explain required inputs and accepted password length clearly while turning safe validation/auth failures into actionable accessible notifications? |
