...
The ticket set does not merge the fixture package into the auth boundary, migrate demo fixtures into production tables, or require the rest of the prototype to become production-backed. The tickets remain `planned` until implementation is explicitly authorized through the normal review controls.
...
- Tickets remain `planned` until explicitly authorized.
