...
## CFC CK-001 status-metric remediation checkpoint

- **Base / review:** `36fe4209611272f717eb41fbd90d749865d918a3`, `PCC-BATCH-05-36fe420-review.md`, session 3 Round 1 `CHANGES_REQUIRED`, finding `CK-001`.
- **User clarification:** the production card's third “Waiting” metric is intentionally authorized to match the three-metric card composition on `/demo`; the metric's value and label must be derived from the same status displayed by the badge, rather than hard-coded independently.
