...
- **State:** `in_progress` — `SIN-001` and `SIN-002` are approved; `SIN-003` / `SIN-BATCH-03` is the authorized active checkpoint.
...
| 3 | [SIN-003](SIN-003-authenticated-home-route-and-navigation.md) / `SIN-BATCH-03` | `in_progress` | SIN-002 `PASS` | Does `/home` resolve the current Better Auth session and render the shared shell with real name/email, empty production projects, route-correct navigation, and no fake logout/role? |
