...
- **State:** `in_progress` — `SIN-001` / `SIN-BATCH-01` is authorized by `go`.
...
|---:|---|---|---|---|
| 1 | [SIN-001](SIN-001-sign-in-form-and-submission.md) / `SIN-BATCH-01` | `in_progress` | BSS-004 approved; SUS set approved/frozen | Does `/sign-in` submit existing credentials through Better Auth and navigate to `/home` only after a successful response, with bounded failure behavior? |
