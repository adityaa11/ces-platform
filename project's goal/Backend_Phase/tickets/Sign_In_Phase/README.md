...
- **State:** `approved` — `SIN-001` through `SIN-004` are approved; the sign-in and authenticated-home ticket set is complete.
...
| 4 | [SIN-004](SIN-004-sign-in-home-end-to-end-validation.md) / `SIN-BATCH-04` | `approved` | SIN-001 through SIN-003 `PASS` | Does the complete sign-in-to-home flow work against the existing auth persistence while `/demo`, CSP, fixture, and authorization boundaries remain intact? |
