...
export interface ProcessResult { readonly exitCode: number; readonly stdout: string; readonly stderr: string }
export interface RunnerDependencies {
  readonly execute: (command: string, args: readonly string[], cwd: string) => Promise<ProcessResult>;
...
export async function runCes(options: RunCesOptions, dependencies: RunnerDependencies = defaultDependencies()): Promise<number> {
  ...
  const checkout = join(runtime, "checkout");
  let invoked = false;
  ...
  try {
    ...
    await prepareCheckout(checkout, lock.ces.commit, options.repository ?? CANONICAL_CES_REPOSITORY, dependencies.execute);
    ...
    const pnpmVersion = await dependencies.execute(pnpmVersionCommand.command, pnpmVersionCommand.args, checkout);
    ...
    const install = corepackInvocation(["pnpm", "install", "--frozen-lockfile"]);
    await requireSuccess(dependencies.execute(install.command, install.args, checkout), "CES_INSTALL_FAILED", "Dependency installation failed.");
    const build = corepackInvocation(["pnpm", "build"]);
    await requireSuccess(dependencies.execute(build.command, build.args, checkout), "CES_BUILD_FAILED", "The pinned CES build failed.");

    const staged = stagingPath(finalOutput);
    ...
    await publish(staged, finalOutput, dependencies.beforePublish);
    ...
    if (!invoked) {
      try { await publishReportOnly(finalOutput, report, dependencies.beforePublish); } catch { /* retain the original failure */ }
    } else await rm(stagingPath(finalOutput), { recursive: true, force: true });
    return 1;
  }
}

async function prepareCheckout(checkout: string, commit: string, repository: string, execute: RunnerDependencies["execute"]): Promise<void> {
  ...
  if (!(await exists(join(checkout, ".git")))) {
    ...
    await requireSuccess(execute("git", ["clone", "--no-checkout", repository, checkout], dirname(checkout)), "CES_CHECKOUT_FAILED", "The canonical CES repository could not be cloned.");
  } else {
    const origin = await execute("git", ["-C", checkout, "remote", "get-url", "origin"], checkout);
  ...
  await requireSuccess(execute("git", ["-C", checkout, "fetch", "--no-tags", "origin", commit], checkout), "CES_CHECKOUT_FAILED", "The locked CES commit could not be fetched.");
  await requireSuccess(execute("git", ["-C", checkout, "checkout", "--detach", commit], checkout), "CES_CHECKOUT_FAILED", "The locked CES commit could not be checked out.");
  const head = await execute("git", ["-C", checkout, "rev-parse", "HEAD"], checkout);
...
async function publishReportOnly(finalOutput: string, report: ExecutionReport, hook?: RunnerDependencies["beforePublish"]): Promise<void> {
  const staged = stagingPath(finalOutput);
  await rm(staged, { recursive: true, force: true });
  await mkdir(staged, { recursive: true });
  await writeReport(staged, ExecutionReportSchema.parse(report));
  await publish(staged, finalOutput, hook);
}

async function publish(staged: string, finalOutput: string, hook?: RunnerDependencies["beforePublish"]): Promise<void> {
  const backup = backupPath(finalOutput);
  ...
  try {
    ...
    if (hadFinal && await exists(backup) && !(await exists(finalOutput))) await rename(backup, finalOutput);
    throw error;
  } finally {
    await rm(staged, { recursive: true, force: true });
  }
...
async function requireSuccess(result: Promise<ProcessResult>, code: string, message: string): Promise<void> { if ((await result).exitCode !== 0) throw new RunnerFailure("execution_error", code, message); }
...
export function defaultDependencies(): RunnerDependencies {
  return {
    ...
    execute: (command, args, cwd) => new Promise((resolveResult) => {
