...
export type SfeExtractionResult = { executionId: string; mode: ExtractionMode; artifact: SfeSourceArtifact; pages: readonly { page: number; text: string }[]; candidateAssertions: readonly SfeCandidate[]; sourceStatementInventory: readonly SfeInventoryEntry[]; questions: readonly Record<string, unknown>[] };
...
export function validateSfeExtraction(project: SfeProjectInput, result: SfeExtractionResult): void {
  ...
  if (!result.pages.length || result.pages.some((page, index) => page.page !== index + 1 || !page.text.trim())) throw new Error("Source pages must be contiguous and non-empty.");
  ...
  if (linkedCandidates.size !== result.candidateAssertions.length || result.pages.some((page) => !result.sourceStatementInventory.some((entry) => entry.page === page.page))) throw new Error("Source accounting is incomplete.");
