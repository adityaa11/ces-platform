...
export type SfeInventoryEntry = { inventoryId: string; artifactId: string; page: number; quote: string; classification: "material" | "non_fact" | "empty_page"; normalizedInterpretation: Record<string, unknown>; destination: { type: "candidate_assertion"; candidateId: string; duplicateOf?: string } | { type: "non_fact"; reason: string; duplicateOf?: string } };
...
const normalizedSourceText = (value: string) => value.replace(/\s+/g, " ").trim();
...
export function validateSfeExtraction(project: SfeProjectInput, result: SfeExtractionResult): void {
  ...
  if (!/^[a-f0-9]{64}$/i.test(result.artifact.sha256) || result.artifact.sha256.toLowerCase() !== result.artifact.verifiedSha256.toLowerCase()) throw new Error("Stored source hash does not match the verified bytes.");
  const stored = project.sourceFiles.find((file) => file.relativePath === result.artifact.relativePath && file.sha256.toLowerCase() === result.artifact.sha256.toLowerCase());
  ...
  for (const entry of result.sourceStatementInventory) {
    ...
    if (entry.classification !== "empty_page" && (!entry.quote || !normalizedSourceText(sourcePage.text).includes(normalizedSourceText(entry.quote)))) throw new Error("Inventory quote is not grounded in its page.");
    if (entry.destination.type === "candidate_assertion") {
      if (!candidates.has(entry.destination.candidateId)) throw new Error("Inventory candidate destination is dangling.");
      if (linkedCandidates.has(entry.destination.candidateId) && entry.destination.duplicateOf !== entry.destination.candidateId) throw new Error("Repeated inventory candidate destinations must identify their duplicate target.");
      linkedCandidates.add(entry.destination.candidateId);
    }
  ...
  for (const candidate of result.candidateAssertions) {
    ...
    if (candidate.evidence.artifactId !== result.artifact.artifactId || !page || !normalizedSourceText(page.text).includes(normalizedSourceText(candidate.evidence.quote))) throw new Error("Candidate evidence is not grounded in the source artifact.");
...
export function completeSfeExtraction(project: SfeProjectInput, result: SfeExtractionResult): CompletedFixtureProject {
  ...
  return { ...project, project: { ...project.project, status: "ready", lastActivity: "Initial Draft is ready for review", repository: { ...project.project.repository, state: "ready-for-review", summary: "Initial Draft extraction is complete and awaiting review.", initialDraft: { processedPrds: project.prdRecord.prdFiles.length, totalPrds: project.prdRecord.prdFiles.length, progress: 100 }, metrics: [{ value: String(result.candidateAssertions.length), label: "extracted facts" }, { value: String(project.prdRecord.prdFiles.length), label: "PRDs uploaded" }, { value: "Ready", label: "to review" }], action: { label: "Review Initial Draft", enabled: true } } }, processingJob: { ...project.processingJob, stage: "ready", message: "Ready to review", progress: 100 }, initialDraftWorkspace: { ...draft, status: "ready-for-review", available: true }, extraction: result };
}
