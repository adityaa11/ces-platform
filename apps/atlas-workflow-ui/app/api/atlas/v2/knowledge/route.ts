...
import { atlasArtifactRoot, authorizeAtlasRequest, readKnowledgeNode } from "../../../../../lib/knowledge-v2";
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json(await readKnowledgeNode({ root: atlasArtifactRoot(), projectId,
      knowledgeId, revision, lifecycle: request.nextUrl.searchParams.get("lifecycle") === "approved"
        ? "approved" : "proposed" }));
  } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Atlas unavailable" },
