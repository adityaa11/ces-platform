...
export function buildSemanticFactRequest(input: Input, modelAlias: string,
  ...
  const focus = input.extraction_focus !== "all"
    ...
    : `Find business capabilities/modules, actors, inputs, activities and order, decisions, conditions and outcomes, rules, states and transitions, entities and relationships, dependencies, events, permissions, validations, audit actions, and nonfunctional requirements. For every fact set engineering_intent and decomposition_status. When evidence explicitly places a fact inside a broader source concept, set parent_source_label to that exact cited wording. Decompose substantive sections into the smallest independently meaningful source-supported facts; do not stop at headings or introductory summaries.`;
  return {
    system_instructions: `Extract atomic semantic facts from this bounded source scope. Inspect every source unit in the request; an empty facts array is valid only when none contains a supported semantic fact. Return the top-level schema_version exactly as "2.0.0". ${focus} Treat document headings as evidence-location cues, not automatically as system behavior. Never assume the document supports a workflow. Preserve exact_statement, parent_source_label, and every term exactly as written in a cited source unit; never translate or paraphrase. Proposed multilingual equivalence remains pending human review. Do not choose graph types or create graph topology. Return JSON only.`,
