...
export function ProfileMenu({ user }: { user: User }) {
  ...
  return <div className="profile-wrap" ref={ref}><button aria-expanded={open} aria-label={`${user.name}, ${user.role}, ${user.email}`} className="profile-control" onClick={() => setOpen((value) => !value)} type="button"><Avatar name={user.name} size="small" /><span><strong>{user.name}</strong><small>{user.role}</small></span></button>{open && (mobile ? <Dialog onClose={() => setOpen(false)} title="Account menu"><div className="profile-sheet">{actions}</div></Dialog> : <div className="profile-menu">{actions}</div>)}</div>;
}
