...
import { demoHref, type WorkspaceLens } from "./WorkspaceLens";
...
export function KnowledgeFactsLayout({ active, approval, facts, heading, initialFactId, isolate, lens, operationalModel, renderExtension, user, workspace }: Props) {
  return <>
    ...
    <div className="facts-refactor-layout mobile-context-layout"><section className="knowledge-list mobile-context-main" aria-label={`${heading.title} groups`}>{facts.map((fact) => {
      ...
      return <details className={active && !matched ? "is-muted" : ""} id={fact.id} key={fact.id} open={fact.id === initialFactId}><summary><span>{fact.number}</span><div><h2>{fact.title}</h2><p>{fact.summary}</p></div><small><b>{rows.length} fact{rows.length === 1 ? "" : "s"}</b>{fact.prdIds.map((id) => workspace.prds.find((prd) => prd.id === id)?.increment).filter(Boolean).join(" · ")}</small></summary><div className="knowledge-list-body">{rows.map((row) => <article className={active && matches(row.prdIds, lens.selectedPrdIds) ? "is-matched" : ""} key={row.id}><div><small>Original PRD wording</small><blockquote>“{row.statement}”</blockquote><p><strong>Atlas interpretation:</strong> {row.evidence[0].understood}</p><details><summary>Read source evidence</summary><blockquote>“{row.evidence[0].quote}”</blockquote><small>{row.evidence[0].documentName} · page {row.evidence[0].page}</small></details>{renderExtension?.(fact, row)}</div><aside><small>Related main workflow</small>{row.relatedWorkflowIds.map((workflowId) => { const workflow = workspace.workflows.find((item) => item.id === workflowId); const group = workspace.workflowGroups.find((item) => item.id === workflow?.groupId); return workflow && <Link href={demoHref({ projectId: workspace.project.id, view: "workflow", workflowId, prd: lens.selectedPrdIds.length ? lens.selectedPrdIds.join(",") : undefined, lens: lens.mode === "isolate" ? "isolate" : undefined })} key={workflowId}><strong>{group?.order} · {group?.title}</strong><span>{workflow.title} →</span></Link>; })}</aside></article>)}</div></details>;
