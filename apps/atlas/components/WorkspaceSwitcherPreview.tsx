...
import { resolveFixtureWorkspaceInventory } from "@atlas/fixtures";
...
export function WorkspaceSwitcherDemoHost({ initialWorkspaceId = workspaceSwitcherPreviewModel.selectedId, projectName, unavailableWorkspaceName }: { initialWorkspaceId?: string; projectName: string; unavailableWorkspaceName?: string }) {
  ...
  const select = (workspaceId: string) => { setSelectedId(workspaceId); const url = new URL(window.location.href); url.searchParams.set("workspaceId", workspaceId); window.history.replaceState(null, "", url); setRouteNotice(""); };
  return <><p aria-live="polite" className="sr-only">{routeNotice}</p><WorkspaceSwitcher model={{ ...workspaceSwitcherPreviewModel, projectName, selectedId }} onSelect={select} /></>;
}
