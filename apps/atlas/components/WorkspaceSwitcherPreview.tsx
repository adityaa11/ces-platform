...
const preview: WorkspaceSwitcherModel = { projectName: "Safara", selectedId: "increment-03", workspaces: [
...
export function WorkspaceSwitcherPreview() { const [selectedId, setSelectedId] = useState(preview.selectedId); return <AppShell active="workflow" projectNavigation projects={[previewProject]} selectedProjectId="safara" sidebarCollapsible={false} user={{ name: "Aditya", email: "aditya@example.com", role: "owner" }} workspaceSwitcher={<WorkspaceSwitcher model={{ ...preview, selectedId }} onSelect={setSelectedId} />}><section className="workspace-switcher-preview"><p className="eyebrow">Main workflow</p><h1>Safara operational model</h1><p>The workspace selector changes which accepted HEAD Atlas treats as current truth without replaying every source PRD.</p></section></AppShell>; }
