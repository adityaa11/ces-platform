...
export function CesResult({ user, projects, workspace, initialLens }: { user: User; projects: ProjectFixture[]; workspace: ProjectWorkspaceFixture; initialLens: Lens }) {
  const { lens, set, toggle } = useWorkspaceLens(initialLens);
  const [approval, setApproval] = useState(workspace.cesApproval);
  ...
  const items = useMemo(() => workspace.cesItems.filter((item) => !active || item.sourcePrdIds.some((id) => lens.selectedPrdIds.includes(id))), [active, lens.selectedPrdIds, workspace.cesItems]);
  ...
  return <AppShell active="ces" projectNavigation projects={projects} routeContext={{ prd: active ? lens.selectedPrdIds.join(",") : undefined, lens: isolate ? "isolate" : undefined }} selectedProjectId={workspace.project.id} topbarAction={<PrdLensControl lens={lens} prds={workspace.prds} set={set} toggle={toggle} />} user={user} workspace={workspace}><section className="knowledge-page ces-page"><KnowledgeFactsLayout active={active} approval={{ status: approval, pendingText: "Review CES assessments against Project Facts before confirming the CES baseline.", approvedText: "The CES baseline has been approved against the accumulated project facts.", reviewLabel: "Review assessments", approveLabel: "Approve CES baseline", onApprove: () => setApproval("approved"), onReview: () => facts[0] && document.getElementById(facts[0].id)?.scrollIntoView({ behavior: "smooth", block: "start" }) }} facts={facts} heading={{ kicker: "CES awareness over project facts", title: "CES Result", description: "The same Project Facts, now with CES interpretation attached to each relevant fact. Expand a fact group to review CES in the source context." }} isolate={isolate} lens={lens} operationalModel={<CesOperationalModel counts={counts} total={items.length} />} renderExtension={(_, row) => <CesAssessmentExtension items={items.filter((item) => item.linkedFactRowIds.includes(row.id))} />} user={user} workspace={workspace} /></section></AppShell>;
}
...
function CesAssessmentExtension({ items }: { items: CesItem[] }) {
  if (!items.length) return null;
  return <section className="ces-layer"><p className="workflow-kicker">CES assessment <span>{items.length} related</span></p>{items.map((item) => <details className={`ces-item coverage-${item.coverage}`} key={item.id}><summary><span>{item.policyId}</span><span className="ces-item-copy"><strong>{item.policy}</strong><small>{item.conclusion}</small></span><em>{item.coverage.replace("-", " ")}</em></summary><dl><div><dt>Policy rule</dt><dd>{item.rule}</dd></div><div><dt>Concern</dt><dd>{item.concern}</dd></div><div><dt>Policy obligation</dt><dd>{item.obligation}</dd></div><div><dt>Capability need</dt><dd>{item.capabilityNeed}</dd></div>{item.decision && <div className="ces-decision"><dt>Open decision</dt><dd>{item.decision}</dd></div>}</dl></details>)}</section>;
}
