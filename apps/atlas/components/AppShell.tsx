...
import type { SignOutMode } from "./ProfileMenu";
...
type Props = { user: AuthenticatedUser; projectRole?: ProjectRole; children: ReactNode; projects: ProjectFixture[]; selectedProjectId?: string; workspace?: ProjectWorkspaceFixture; active?: Destination; projectNavigation?: boolean; sidebarAction?: ReactNode; topbarAction?: ReactNode; workspaceSwitcher?: ReactNode; contentClassName?: string; sidebarCollapsible?: boolean; fullWidthSearch?: boolean; homeHref?: string; signOutMode?: SignOutMode; routeContext?: { scenario?: string; workspaceId?: string; prd?: string; lens?: "isolate" } };
...
function AtlasBrand({ href }: { href: string }) {
  return <Link className="brand" href={href}><span>A</span>Atlas</Link>;
}

export function AppShell({ user, projectRole, children, projects, selectedProjectId, active = "projects", projectNavigation = false, sidebarAction, topbarAction, workspaceSwitcher, contentClassName = "", sidebarCollapsible = true, fullWidthSearch = false, homeHref = "/demo", signOutMode = "fixture-link", routeContext }: Props) {
  ...
  return <div className={`app-shell ${collapsed ? "sidebar-collapsed" : ""}`}>
    ...
    <aside aria-hidden={compact && !collapsed ? true : undefined} aria-label="Workspace navigation" className={`navigation ${collapsed ? "navigation-open" : ""}`} id="app-navigation" inert={compact && !collapsed ? true : undefined} ref={navigationRef}>
      {sidebarCollapsible && <button aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"} className="sidebar-toggle" onClick={() => setCollapsed(!collapsed)} type="button"><svg aria-hidden="true" fill="none" viewBox="0 0 24 24"><rect height="18" rx="2.5" stroke="currentColor" strokeWidth="1.7" width="18" x="3" y="3" /><path d="M9 3v18" stroke="currentColor" strokeWidth="1.7" /><path d={collapsed ? "m13 9 3 3-3 3" : "m16 9-3 3 3 3"} stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.7" /></svg></button>}
      <div className="drawer-heading"><AtlasBrand href={homeHref} /><button aria-label="Close navigation menu" className="drawer-close" onClick={closeNavigation} ref={closeTriggerRef} type="button">×</button></div>
      <section className="sidebar-project-region"><p className="switch-label">Project</p><div className="project-switcher" ref={ref}>
