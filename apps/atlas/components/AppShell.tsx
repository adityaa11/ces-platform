...
export function AppShell({ user, children, projects, selectedProjectId, workspace, active = "projects", projectNavigation = false, sidebarAction, topbarAction }: Props) {
  ...
  return <div className={`app-shell ${collapsed ? "sidebar-collapsed" : ""}`}>
    ...
    <aside className={`navigation ${collapsed ? "navigation-open" : ""}`} id="app-navigation"><button aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"} className="sidebar-toggle" onClick={() => setCollapsed(!collapsed)} type="button">‹</button>
      ...
      <div className="sidebar-action">{sidebarAction}</div>{projectNavigation && <nav aria-label="Project navigation">{(["workflow", "facts", "ces", "changes"] as const).map((view) => <Link aria-current={active === view ? "page" : undefined} className={active === view ? "nav-active" : ""} href={href(view)} key={view}>{view === "workflow" ? "Main Workflow" : view === "facts" ? "Project Facts" : view === "ces" ? "CES Result" : "Changes Done"}</Link>)}</nav>}<ProfileMenu user={user} /></aside><main className="app-content">{children}</main>
  </div>;
}
