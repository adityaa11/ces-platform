...
type Destination = "projects" | "workflow" | "facts" | "changes";
...
export function AppShell({ user, children, projects, selectedProjectId, workspace, active = "projects", projectNavigation = false, sidebarAction, topbarAction }: Props) {
  ...
  useEffect(() => { const close = (event: KeyboardEvent) => { if (event.key === "Escape") setMenu(false); }; document.addEventListener("keydown", close); return () => document.removeEventListener("keydown", close); }, []);
  ...
  return <div className={`app-shell ${collapsed ? "sidebar-collapsed" : ""}`}>
    <TopBar className="app-header" variant="workspace"><Link className="brand" href="/demo"><span>A</span>Atlas</Link><div className="topbar-actions">{topbarAction}<button aria-controls="app-navigation" aria-expanded={collapsed} aria-label={collapsed ? "Close menu" : "Open menu"} className="mobile-menu" onClick={() => setCollapsed(!collapsed)} type="button">Menu</button></div></TopBar>
    <aside className={`navigation ${collapsed ? "navigation-open" : ""}`} id="app-navigation"><button aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"} className="sidebar-toggle" onClick={() => setCollapsed(!collapsed)} type="button">‹</button><ProfileMenu user={user} />
      <div className="project-switcher" ref={ref}><button aria-expanded={menu} onClick={() => setMenu(!menu)} type="button"><span>{current ? current.name[0] : "⌁"}</span><div><small>{current ? "Current project" : "Project"}</small><strong>{current ? current.name : "Select a project"}</strong><em>{current ? `${current.prdCount} PRDs${workflowCount === undefined ? "" : ` · ${workflowCount} workflow pages`}` : `${projects.length} available projects`}</em></div><i>⌄</i></button>{menu && <div className="project-switcher-menu"><header><strong>All projects</strong><small>{projects.length} projects</small></header>{projects.map((project) => { const isCurrent = current?.id === project.id; const detail = <><span>{project.name[0]}</span><div><strong>{project.name}</strong><small>{project.prdCount} PRDs · {project.status === "ready" ? "ready to review" : project.status}</small></div>{isCurrent && <em>Current</em>}</>; return project.status === "ready" ? <Link className={isCurrent ? "is-current" : ""} href={`/demo?projectId=${project.id}&view=workflow`} key={project.id} onClick={() => setMenu(false)}>{detail}</Link> : <span className="is-unavailable" key={project.id}>{detail}</span>; })}<p>Select a ready project to inspect its workflow.</p></div>}</div>
      <div className="sidebar-action">{sidebarAction}</div>{projectNavigation && <nav aria-label="Project navigation">{(["workflow", "facts", "changes"] as const).map((view) => <Link aria-current={active === view ? "page" : undefined} className={active === view ? "nav-active" : ""} href={href(view)} key={view}>{view === "workflow" ? "Main Workflow" : view === "facts" ? "Project Facts" : "Changes Done"}</Link>)}</nav>}</aside><main className="app-content">{children}</main>
  </div>;
}
