...
export function WorkflowWorkspace({ user, projects, workspace, initialLens, initialWorkflowId, initialWorkspaceId, scenario, unavailableWorkspaceName }: { user: User; projects: ProjectFixture[]; workspace: ProjectWorkspaceFixture; initialLens: WorkspaceLens; initialWorkflowId?: string; initialWorkspaceId?: string; scenario?: string; unavailableWorkspaceName?: string }) {
  ...
  return <AppShell active="workflow" projectNavigation projects={projects} routeContext={{ scenario, workspaceId: initialWorkspaceId, prd: active ? lens.selectedPrdIds.join(",") : undefined, lens: isolate ? "isolate" : undefined }} selectedProjectId={workspace.project.id} topbarAction={<PrdLensControl lens={lens} prds={workspace.prds} set={set} toggle={toggle} />} user={user} workspace={workspace} workspaceSwitcher={<WorkspaceSwitcherDemoHost initialWorkspaceId={initialWorkspaceId} projectId={workspace.project.id} projectName={workspace.project.name} unavailableWorkspaceName={unavailableWorkspaceName} />}>
