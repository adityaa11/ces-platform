import { getFixtureScenario, projectCardStressFixtures, resolveFixtureProjectRoute, resolveFixtureWorkspaceContent, resolveFixtureWorkspaceRoute, type FixtureScenario } from "@atlas/fixtures";
...
export default async function DemoPage({ searchParams }: { searchParams?: Promise<{ scenario?: string; stress?: string; view?: string; projectId?: string; workspaceId?: string; prd?: string; lens?: string; workflowId?: string; factId?: string; cesItemId?: string }> }) {
  ...
  if (projectRoute.canOpenWorkspace && selectedWorkspace && requestedView === "workflow") return <WorkflowWorkspace initialLens={initialLens} initialWorkflowId={params?.workflowId} initialWorkspaceId={workspaceRoute?.selectedWorkspace?.workspaceId} projects={routeProjects} scenario={scenarioId} unavailableWorkspaceName={workspaceRoute?.unavailableWorkspace?.name} user={scenario.session} workspace={selectedWorkspace} />;
  if (projectRoute.canOpenWorkspace && selectedWorkspace && (requestedView === "facts" || requestedView === "changes")) return <ProjectKnowledge initialFactId={params?.factId} initialLens={initialLens} initialWorkspaceId={workspaceRoute?.selectedWorkspace?.workspaceId} projects={routeProjects} scenario={scenarioId} unavailableWorkspaceName={workspaceRoute?.unavailableWorkspace?.name} user={scenario.session} view={requestedView} workspace={selectedWorkspace} />;
  if (projectRoute.canOpenWorkspace && selectedWorkspace && requestedView === "ces") return <CesResult initialCesItemId={params?.cesItemId} initialLens={initialLens} initialWorkspaceId={workspaceRoute?.selectedWorkspace?.workspaceId} projects={routeProjects} scenario={scenarioId} unavailableWorkspaceName={workspaceRoute?.unavailableWorkspace?.name} user={scenario.session} workspace={selectedWorkspace} />;
  if (projectRoute.canOpenWorkspace && selectedWorkspace && requestedView === "sources") return <SourcesWorkspace initialLens={initialLens} initialWorkspaceId={workspaceRoute?.selectedWorkspace?.workspaceId} projects={routeProjects} scenario={scenarioId} unavailableWorkspaceName={workspaceRoute?.unavailableWorkspace?.name} user={scenario.session} workspace={selectedWorkspace} />;
  return <ProjectLibrary mode="fixture" projectRole={scenario.session.role} projects={projectCardStress ? projectCardStressFixtures : scenario.projects} scenario={scenarioId} user={scenario.session} workspace={projectCardStress ? undefined : scenario.workspace} />;
}
