...
const projectFixtureRegistryPath = resolve(workspaceRoot, "packages/atlas-fixtures/generated/project-card-fixtures.json");
const safeSegment = (value: string) => /^[a-z0-9][a-z0-9-]{2,47}$/.test(value);
type PersistedModalProject = { files?: unknown; project: ProjectFixture; initialDraftWorkspace: { workspaceId: string; prdFiles: Array<{ name: string; type: "application/pdf"; size: number }> }; sourceFiles?: unknown; [key: string]: unknown };
type ProjectFixtureRegistry = { version: 1; cards: ProjectFixture[]; modalProjects: PersistedModalProject[] };
const initialProjectFixtureRegistry = (): ProjectFixtureRegistry => ({ version: 1, cards: getFixtureScenario("owner-ready").projects.map((project) => structuredClone(project)), modalProjects: [] });
async function readProjectFixtureRegistry(): Promise<ProjectFixtureRegistry> {
  try { return JSON.parse(await readFile(projectFixtureRegistryPath, "utf8")) as ProjectFixtureRegistry; }
  ...
  configureServer(server) {
    server.middlewares.use(async (request, response, next) => {
      ...
      try {
        ...
        if (!Array.isArray(body.request?.prdFiles) || !Array.isArray(body.files) || body.files.length !== body.request.prdFiles.length || body.request.prdFiles.length !== body.project.prdCount) throw new Error("PDF metadata and bytes are required for every selected file.");
        ...
        for (const file of body.files) {
          ...
          const bytes = Buffer.from(file.base64, "base64"); const metadata = body.request.prdFiles.find((item: { name: string }) => item.name === file.name); if (!metadata || metadata.size !== bytes.length) throw new Error("PDF metadata does not match its selected bytes.");
