...
Publication uses a temporary sibling, validates and reports only files present
in that execution, moves any final directory to a backup, promotes the complete
temporary directory, removes the backup on success, and restores it on failure.
Thus separate executions are never mixed. Completed Phase 1 reports contain no
timestamps, durations, absolute paths, temporary names, or random identifiers.
...
`CES_ARTIFACT_MISSING`. Human-readable stderr is never parsed for semantics.
